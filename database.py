...
def add_task(user_id: int, name: str, emoji: str = "✅", duration_minutes: Optional[int] = None) -> int:
    with get_db() as conn:
        cur = conn.execute(
            "INSERT INTO tasks (user_id, name, emoji, duration_minutes) VALUES (?, ?, ?, ?)",
...
def update_task(task_id: int, user_id: int, name: Optional[str] = None, duration_minutes: Optional[int] = None) -> None:
    ...
    with get_db() as conn:
        ...
        if duration_minutes is not None:
            ...
            conn.execute(
                "UPDATE tasks SET duration_minutes = ? WHERE id = ? AND user_id = ?",
...
def start_session(user_id: int, task_id: int, date_str: str) -> int:
    ...
    with get_db() as conn:
        cur = conn.execute(
            "INSERT INTO task_sessions (task_id, user_id, started_at, date) VALUES (?, ?, ?, ?)",
...
def end_session(session_id: int) -> int:
    ...
    with get_db() as conn:
        row = conn.execute(
            "SELECT started_at FROM task_sessions WHERE id = ?", (session_id,)
        ).fetchone()
        if not row:
            return 0
...
def get_today_session_totals(user_id: int, date_str: str) -> dict[int, int]:
    ...
    with get_db() as conn:
        rows = conn.execute(
            ...
            FROM task_sessions
            WHERE user_id = ? AND date = ? AND ended_at IS NOT NULL
            GROUP BY task_id
            """,
            (user_id, date_str),
        ).fetchall()
